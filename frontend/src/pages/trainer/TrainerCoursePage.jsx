...
export default function TrainerCoursePage() {
  ...
  const owningManhattanOnly = pathname.includes('trainer-owning-manhattan');
  ...
  const liveCoursesCount = catalogCourses.filter((course) => String(course.delivery_mode || '').toLowerCase() === 'live').length;
  const filteredCourses = useMemo(() => {
    const base = owningManhattanOnly
      ? catalogCourses
      : activeTab === 'short'
        ? catalogCourses.filter((course) => isShortCourse(course))
        : catalogCourses;
  ...
  }, [activeTab, catalogCourses, searchTerm, owningManhattanOnly]);

  return (
    <TrainerDashboardSectionPage title={owningManhattanOnly ? 'Owning Manhattan' : 'Trainer Course'}>
      <div className="container-fluid px-0" style={{ maxWidth: 1200 }}>
        <div className="lms-card p-4 p-md-5 mb-3 text-white" style={{ background: 'linear-gradient(90deg,#071d3d,#0d2f69 45%,#0a5dea)' }}>
          <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div>
              ...
              <h1 className="h2 fw-bold mb-1">{owningManhattanOnly ? 'Owning Manhattan courses' : 'Trainer Course Panel'}</h1>
              <p className="mb-0 text-light">
                {owningManhattanOnly
                  ? 'Courses tagged as Owning Manhattan for your organization.'
                  : 'Review assigned courses and open detail page for videos and lessons.'}
              </p>
            </div>
          ...
          <div className="row g-3">
            <div className="col-12 col-md-4">
              <div className="border rounded-3 p-3 h-100">
                ...
                <h3 className="h5 mb-0">{catalogCourses.length}</h3>
              </div>
            </div>
            <div className="col-12 col-md-4">
              <div className="border rounded-3 p-3 h-100">
        ...
        <div className="lms-card p-0 mb-3 overflow-hidden">
          <div className="px-3 py-2 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            {owningManhattanOnly && <span className="fw-semibold text-body">Owning Manhattan only</span>}
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="text-center py-5 text-muted">
                      {activeTab === 'short' && !owningManhattanOnly
                        ? 'No short courses found.'
                        : owningManhattanOnly
                          ? 'No Owning Manhattan courses found.'
                          : 'No courses found.'}
                    </td>
                  </tr>
                ) : (
                      ...
                      <td className="text-end pe-4">
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              owningManhattanOnly
                                ? `/dashboard/trainer-course/${course.id}?from=owning-manhattan`
                                : `/dashboard/trainer-course/${course.id}`,
                            )
                          }
                        >
                          View
