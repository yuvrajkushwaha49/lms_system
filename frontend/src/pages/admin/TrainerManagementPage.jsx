...
export default function TrainerManagementPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const trainerUsers = (payload.data || []).filter((user) => {
  ...
  const handleCreateUser = async (event) => {
    ...
    try {
      ...
      if (!response.ok || payload.status !== 'success') {
        throw new Error(payload.message || 'Unable to create trainer');
      }

      setFeedback('Trainer created successfully.');
  ...
  const handleEditUser = async (event) => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok || payload.status !== 'success') {
        throw new Error(payload.message || 'Unable to update trainer');
      }
      setFeedback('Trainer updated successfully.');
  ...
  const handleDeleteUser = async (user) => {
    if (!window.confirm(`Delete trainer "${user.name}"?`)) return;
    setFeedback('');
    setError('');
    const token = localStorage.getItem('token');
    if (!token) {
    ...
    try {
      ...
      setFeedback('Trainer deleted successfully.');
      await fetchUsers();
    } catch (deleteError) {
  ...
  const handleToggleUserStatus = async (user) => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok || payload.status !== 'success') {
        throw new Error(payload.message || 'Unable to update trainer status');
      }
      setFeedback(payload?.data?.status === 'active' ? 'Trainer activated.' : 'Trainer deactivated.');
      await fetchUsers();
    } catch (statusError) {
        ...
        <div className="lms-card p-4 p-md-5 mb-3 text-white" style={{ background: 'linear-gradient(90deg,#071d3d,#0d2f69 45%,#0a5dea)' }}>
          <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div>
              ...
              <h1 className="h2 fw-bold mb-1">Trainer Management</h1>
              <p className="mb-0 text-light">Manage trainers, roles, and account status.</p>
            </div>
                ...
                {isLoadingUsers ? (
                  ...
                  filteredUsers.map((user) => (
                    <tr key={`${user.id}-${user.email}`}>
                      ...
                      <td><span className="badge text-bg-info">{user.role_name || user.role || 'Instructor'}</span></td>
                      <td>
                        <span className={`badge text-uppercase ${(user.status || 'active') === 'active' ? 'text-bg-success' : 'text-bg-secondary'}`}>
                          {user.status || 'active'}
                        </span>
                      </td>
                      ...
                      <td className="text-end pe-4">
                        <div className="d-flex justify-content-end flex-wrap gap-2">
                          <button type="button" onClick={() => openEditModal(user)} className="btn btn-outline-primary btn-sm">
                            Edit
                          </button>
                          <button
                            type="button"
                          ...
                          <button type="button" onClick={() => handleDeleteUser(user)} className="btn btn-outline-danger btn-sm">
                            Delete
                          </button>
                        </div>
                      </td>
            ...
            <div className="card shadow-lg border-0" style={{ width: '100%', maxWidth: 520, borderRadius: 18 }}>
              <div className="card-body p-4">
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div>
                      <h2 className="h4 mb-1">Add trainer</h2>
                    <p className="text-muted mb-0 small">New trainers are created with Instructor role only.</p>
                  </div>
          ...
          <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ background: 'rgba(15,23,42,0.45)', zIndex: 1200 }}>
            <div className="card shadow-lg border-0" style={{ width: '100%', maxWidth: 520, borderRadius: 18 }}>
              <div className="card-body p-4">
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div>
                    <h2 className="h4 mb-1">Edit trainer</h2>
                    <p className="text-muted mb-0 small">Update trainer details.</p>
                  </div>
                ...
                <form onSubmit={handleEditUser}>
                  <div className="mb-3">
                    <label className="form-label">Name</label>
                    <input type="text" value={editFormData.name} onChange={(event) => setEditFormData((prev) => ({ ...prev, name: event.target.value }))} required className="form-control" placeholder="Full name" />
                  </div>
                  <div className="mb-3">
                    <label className="form-label">Email</label>
                    <input type="email" value={editFormData.email} onChange={(event) => setEditFormData((prev) => ({ ...prev, email: event.target.value }))} required className="form-control" placeholder="name@company.com" />
                  </div>
                  <div className="mb-3">
                    <label className="form-label">Phone</label>
                    <input type="text" value={editFormData.phone} onChange={(event) => setEditFormData((prev) => ({ ...prev, phone: event.target.value }))} required className="form-control" placeholder="9876543210" />
                  </div>
                  <div className="d-flex justify-content-end gap-2">
                    <button type="button" onClick={() => setShowEditModal(false)} className="btn btn-outline-secondary">Cancel</button>
